...
Real people are tracked in `src/data/people.ts` (`styret` and `crew` arrays, photos imported from `src/assets/crew/*.webp`) and rendered in `kontakt.astro`. Any `CrewName` with no matching entry in `crew`, and any of the 6 Styret seats beyond `styret.length`, still fall back to a placeholder `PersonCard` (name defaults to "Navn kommer", email/phone read "E-post kommer"/"Telefon kommer"). `PersonCard.astro` renders a generic inline-SVG person-silhouette avatar when no `photo` prop is passed — deliberately generic rather than reusing a game illustration, so it reads unambiguously as "photo not yet supplied" rather than a themed choice. `"Core"` crew is included in the public listing (client confirmed 2026-08-05, superseding an earlier placeholder-era exclusion) once a lead has a photo — don't re-exclude it. As more people are confirmed, add entries to `people.ts` rather than editing `kontakt.astro`'s markup. Source photos arrive named `Name - Crew - Phone - Email.png`; convert to `.webp` (matching the competition-photo convention, `cwebp -q 88`) and rename to a `first-last.webp` slug when copying into `src/assets/crew/` — never commit a filename carrying a phone number or email address.
